...
/*      
        ...
        Returns the kind of the cursor as a string
*/
...
static int cursor_getbitfield_width(lua_State *L)
{
        ...
        lua_pushnumber(L, bitfield_width);
        return 1;
}
...
/*
        ...
        More info - https://clang.llvm.org/doxygen/group__CINDEX__TYPES.html#gac9d37f61bede521d4f42a6553bcbc09f
        Returns the spelling of the function's return type
*/
static int type_getresult(lua_State *L)
{
        ...
        CXString result_str = clang_getTypeSpelling(result);
        lua_pushstring(L, clang_getCString(result_str));
        clang_disposeString(result_str);
        return 1;
}
...
{
        if(luaL_newmetatable(L, name) == 0) luaL_getmetatable(L, name);
