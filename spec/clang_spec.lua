...
describe("cursor_type:getArrayType()", function()
        it("obtains the expected array type", function()
                ...
                local arr_type
                assert.has_no.errors(function()
                        arr_type = cursor_type:getArrayType()
                end)
        ...
        it("uses an incompatible type object with getArrayType()", function()
                local parser = luaclang.newParser("spec/pointer.c")
                local cursor = parser:getCursor()
                cursor = get_last_child(cursor)
                local cursor_type = cursor:getType()
                assert.has.errors(function()
                        local arr_type = cursor_type:getArrayType()
                end, "calling 'getArrayType' on bad self (expect type object with array kind)")
                parser:dispose()
        end)
end)
